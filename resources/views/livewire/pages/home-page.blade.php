<main class="homepage-shell">
    <section class="hero-aurora relative overflow-hidden">
        <div class="pointer-events-none absolute inset-0 z-0 bg-cover bg-center opacity-30 hero-parallax"
            style="background-image: url('{{ asset('images/portfolioimage.jpg') }}');"></div>
        <div
            class="pointer-events-none absolute -top-12 left-0 h-64 w-64 rounded-full bg-cyan-400/20 blur-3xl float-slow">
        </div>
        <div
            class="pointer-events-none absolute -bottom-10 right-5 h-72 w-72 rounded-full bg-blue-500/20 blur-3xl float-delay">
        </div>

        <div
            class="relative z-10 mx-auto grid max-w-7xl gap-10 px-4 py-24 sm:px-6 lg:grid-cols-2 lg:items-center lg:px-8">
            <div data-animate>
                @if (session('success'))
                ...
                <p
                    class="inline-flex rounded-full border border-cyan-300/40 bg-cyan-400/10 px-3 py-1 text-xs font-medium uppercase tracking-[0.2em] text-cyan-100">
                ...
                <div class="mt-8 flex flex-wrap items-center gap-3">
                    <button type="button" wire:click="openProjectStarter"
                        class="inline-flex rounded-md bg-blue-600 px-5 py-3 text-sm font-semibold text-white transition hover:-translate-y-0.5 hover:bg-blue-500">
                        Start a Project
                    </button>
                    @if ($caseStudies->isNotEmpty())
                        <button type="button" wire:click="openCaseStudies"
                            class="inline-flex rounded-md border border-white/30 bg-white/10 px-5 py-3 text-sm font-semibold text-white transition hover:bg-white/20">
    ...
    @if ($showProjectStarter)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-4"
            wire:key="project-starter-modal">
            <div class="w-full max-w-2xl rounded-2xl border border-gray-700 bg-gray-900 p-6 shadow-2xl">
                <div class="flex items-start justify-between gap-4">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-300">Share your details and project brief to submit directly
                            from here.</p>
                    </div>
                    <button type="button" wire:click="closeProjectStarter"
                        class="rounded bg-gray-800 px-3 py-1 text-sm text-gray-200 hover:bg-gray-700">Close</button>
                </div>
                ...
                <form wire:submit="submitProjectStarter" class="mt-5 space-y-4">
                    <div class="grid gap-4 sm:grid-cols-2">
                        <div>
                            ...
                            <input wire:model="name" type="text"
                                class="w-full rounded-md border border-gray-700 bg-gray-800 px-3 py-2 text-white">
                            @error('name')
                    ...
                    <div>
                        ...
                        <textarea wire:model="goals" rows="5"
                            class="w-full rounded-md border border-gray-700 bg-gray-800 px-3 py-2 text-white"></textarea>
                        @error('goals')
                            <p class="mt-1 text-xs text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
                    <div class="flex justify-end">
                        <button type="submit"
                            class="rounded-md bg-blue-600 px-5 py-2 text-sm font-semibold text-white hover:bg-blue-500">
    ...
    <section id="about" class="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
        <div class="grid gap-8 lg:grid-cols-[1.1fr_0.9fr] lg:items-start">
            <div data-animate>
                ...
                <h2 class="mt-2 text-3xl font-semibold text-white">A Builder Focused on Outcomes, Not Just Features
                </h2>
                @if ($about)
                ...
                @else
                    ...
                    @auth
                        <a wire:navigate href="{{ route('abouts.create') }}"
                            class="mt-4 inline-flex rounded-md bg-blue-600 px-4 py-2 text-sm font-medium hover:bg-blue-500">
                            Add About
                        </a>
                    @endauth
                @endif
            ...
            <div class="grid gap-4 sm:grid-cols-2" data-animate>
                <article class="rounded-2xl border border-gray-700 bg-gray-900/80 p-5 card-lift">
                    ...
                    <p class="mt-2 text-sm text-gray-300">From architecture to deployment, I build maintainable apps
                        designed for long-term growth.</p>
                </article>
                <article class="rounded-2xl border border-gray-700 bg-gray-900/80 p-5 card-lift">
                    <h3 class="text-lg font-semibold text-white">EdTech Systems</h3>
                    <p class="mt-2 text-sm text-gray-300">Learning workflows, progress tracking, and interactive
                        content that improve completion rates.</p>
                </article>
                <article class="rounded-2xl border border-gray-700 bg-gray-900/80 p-5 card-lift">
                    <h3 class="text-lg font-semibold text-white">Performance UX</h3>
                    <p class="mt-2 text-sm text-gray-300">Fast-loading interfaces with smooth micro-interactions and
                        clean information hierarchy.</p>
                </article>
                <article class="rounded-2xl border border-gray-700 bg-gray-900/80 p-5 card-lift">
                    <h3 class="text-lg font-semibold text-white">Business Alignment</h3>
                    <p class="mt-2 text-sm text-gray-300">Every decision ties back to conversion, retention,
                        reliability, and measurable value.</p>
                </article>
            </div>
        </div>
    </section>
        ...
        <div class="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
            ...
            <div class="mt-10 grid gap-6 md:grid-cols-3">
                <article data-animate class="rounded-2xl border border-gray-700 bg-gray-900 p-6 step-glow">
                    ...
                    <p class="mt-2 text-sm text-gray-300">Clarify goals, users, and success metrics so we solve the
                        right problem first.</p>
                </article>
                <article data-animate class="rounded-2xl border border-gray-700 bg-gray-900 p-6 step-glow">
                    ...
                    <p class="mt-2 text-sm text-gray-300">Ship scalable backend logic and responsive interfaces with
                        clear iteration cycles.</p>
                </article>
                <article data-animate class="rounded-2xl border border-gray-700 bg-gray-900 p-6 step-glow">
                    ...
                    <p class="mt-2 text-sm text-gray-300">Use analytics and feedback loops to improve engagement,
                        speed, and outcomes.</p>
                </article>
            </div>
        </div>
    </section>
        ...
        <section id="case-studies" class="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
            <div class="flex flex-wrap items-end justify-between gap-4" data-animate>
                ...
                <a wire:navigate href="{{ route('projects.index') }}"
                    class="inline-flex rounded-md border border-white/30 bg-white/10 px-4 py-2 text-sm font-medium text-white hover:bg-white/20">
                ...
                <div class="mt-8 grid gap-6 lg:grid-cols-[1.15fr_0.85fr]" data-animate>
                    <article class="overflow-hidden rounded-2xl border border-gray-700 bg-gray-900/90 card-lift">
                        @if ($selectedCaseStudy->cover_image)
                            <img src="{{ asset('storage/' . $selectedCaseStudy->cover_image) }}"
                                alt="{{ $selectedCaseStudy->title }}" class="h-72 w-full object-cover sm:h-96">
                        @endif
                        <div class="p-6">
                            <h3 class="text-2xl font-semibold text-white">{{ $selectedCaseStudy->title }}</h3>
                            <p class="mt-3 line-clamp-5 text-gray-300">
                                {{ $selectedCaseStudy->excerpt ?: 'No summary provided yet.' }}</p>
                            <div class="mt-5 flex flex-wrap gap-3">
                                <a wire:navigate href="{{ route('case-studies.show', $selectedCaseStudy) }}"
                                    class="rounded-md bg-blue-600 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-500">
                                    Open Case Study
                                </a>
                                @if ($selectedCaseStudy->project_url)
                                    <a href="{{ $selectedCaseStudy->project_url }}" target="_blank"
                                        class="rounded-md border border-gray-600 px-4 py-2 text-sm font-semibold text-gray-200 hover:bg-gray-800">
                                        Visit Live
                                    </a>
                                @endif
                            </div>
                    ...
                    <div class="space-y-3">
                        @foreach ($caseStudies as $caseStudy)
                            <button type="button" wire:click="selectCaseStudy({{ $caseStudy->id }})"
                                ...
                                <p class="mt-1 line-clamp-2 text-sm text-gray-300">
                                    {{ $caseStudy->excerpt ?: 'No summary provided yet.' }}</p>
                            </button>
                        @endforeach
                    </div>
                </div>
        ...
        <div class="mt-8 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
            @forelse ($projects as $project)
                <article data-animate
                    class="group overflow-hidden rounded-2xl border border-gray-700 bg-gray-900/80 card-lift">
                    @php
                    ...
                    <div class="relative overflow-hidden">
                        @if (count($images) > 0)
                            <img src="{{ asset('storage/' . $images[0]) }}" alt="{{ $project->title }}"
                                class="h-52 w-full object-cover transition duration-700 group-hover:scale-110">
                        @elseif (count($videos) > 0)
                            <video controls preload="metadata" playsinline class="h-52 w-full bg-black object-cover">
                                <source src="{{ asset('storage/' . $videos[0]) }}" type="video/mp4">
                            </video>
                        @else
                            <div
                                class="flex h-52 w-full items-center justify-center bg-gray-950 text-sm text-gray-400">
                                No media available
                            </div>
                        @endif
            ...
            <div class="mx-auto max-w-4xl px-4 py-16 sm:px-6 lg:px-8">
                ...
                <div class="testimonial-stage mt-8 rounded-2xl border border-gray-700 bg-gray-900/90 p-8">
                    ...
                    <div class="mt-6 flex justify-center gap-2">
                        @foreach ($testimonials as $testimonial)
                            <button type="button"
                                class="testimonial-dot {{ $loop->first ? 'is-active' : '' }} h-2.5 w-2.5 rounded-full bg-gray-600"></button>
                        @endforeach
                    </div>
                </div>
            </div>
    ...
    <section id="contact" class="relative overflow-hidden">
        ...
        <div class="mx-auto grid max-w-7xl gap-10 px-4 py-16 sm:px-6 lg:grid-cols-[0.95fr_1.05fr] lg:px-8">
            ...
            <form wire:submit="submitContact"
                class="space-y-4 rounded-2xl border border-gray-700 bg-gray-900/90 p-6 shadow-2xl shadow-cyan-900/10"
                data-animate>
                <div>
                    <label for="name" class="mb-1 block text-sm text-gray-300">Name</label>
                    <input wire:model="name" id="name" type="text"
                        class="w-full rounded-md border border-gray-700 bg-gray-800 px-3 py-2 text-white focus:border-cyan-400 focus:outline-none">
                    @error('name')
                ...
                <div>
                    ...
                    <textarea wire:model="message" id="message" rows="5"
                        class="w-full rounded-md border border-gray-700 bg-gray-800 px-3 py-2 text-white focus:border-cyan-400 focus:outline-none"></textarea>
                    @error('message')
                        <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                    @enderror
                </div>
                <button type="submit"
                    class="inline-flex rounded-md bg-blue-600 px-5 py-2 font-semibold text-white transition hover:-translate-y-0.5 hover:bg-blue-500">
                    Send Message
                </button>
            </form>
        </div>
