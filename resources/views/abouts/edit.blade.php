            ...
            <section id="about" class="py-16 bg-gray-900">
              <div class="container mx-auto px-6">
                <form action="{{ route('abouts.update', $about->id) }}" method="POST" class="mt-8 max-w-3xl mx-auto">
                  @csrf
                  @method('PUT')
                  <textarea name="content" class="w-full bg-gray-800 text-gray-300 p-4 rounded @error('content') border border-red-500 @enderror" rows="6">{{ old('content', $about->content ?? '') }}</textarea>
                  @error('content')
